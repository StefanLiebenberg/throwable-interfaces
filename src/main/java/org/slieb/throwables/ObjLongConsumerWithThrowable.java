...
default java.util.function.ObjLongConsumer<T> thatDoesNothing() {
   return (v1, v2) -> {
     ...
     * 
     */
    default ObjLongConsumerWithThrowable<T, E> withLogging(java.util.logging.Logger logger, java.util.logging.Level level) {
        return (v1, v2) -> {
            try {
                acceptWithThrowable(v1, v2);
            } catch (final Throwable throwable) {
                logger.log(level, "exception in ObjLongConsumerWithThrowable", throwable);
                throw throwable;
            }
        };
    }
