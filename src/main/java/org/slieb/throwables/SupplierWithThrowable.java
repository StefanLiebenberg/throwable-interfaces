...
public interface SupplierWithThrowable<T, E extends Throwable> extends java.util.function.Supplier<T> {
    ...
    default SupplierWithThrowable<T, E> withLogging(java.util.logging.Logger logger, java.util.logging.Level level, String message) {
        return () -> {
            try {
                return getWithThrowable();
            } catch (final Throwable throwable) {
    ...
    default SupplierWithThrowable<T, E> withLogging(java.util.logging.Logger logger) {
        return withLogging(logger, java.util.logging.Level.WARNING, "Exception in SupplierWithThrowable");
    }
